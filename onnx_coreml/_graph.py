...
class Graph(object):
    def __init__(self,
        ...
        self.blob_to_op_type = {} # type: Dict[Text, Sequence[Text]]
    ...
    def from_onnx(graph):  # type: (GraphProto) -> Graph
        ...
        # data blob name to the list of op types it feeds into
        blob_to_op_type = {} # type: Dict[Text, Sequence[Text]]
        # data blob name to the op_type that generates it
        blob_from_op_type = {} # type: Dict[Text,Text]

        for node_ in nodes_:
            for input_ in node_.inputs:
                if input_ in blob_to_op_type:
                    blob_to_op_type[input_].append(node_.op_type)
                else:
                    blob_to_op_type[input_] = [node_.op_type]
                if input_ in nodes_by_output:
                    node_.parents.append(nodes_by_output[input_])
            for output_ in node_.outputs:
                if output_ in blob_from_op_type: raise ValueError("Data blob: %s, is generated by more than 1 op" %(output_))
                blob_from_op_type[output_] = node_.op_type
